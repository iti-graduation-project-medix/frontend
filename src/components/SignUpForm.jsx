...
export function SignUpForm({ className, ...props }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        formData.append("dateOfBirth", values.dateOfBirth ? values.dateOfBirth.toISOString() : "");
  ...
  const handleFileChange = (e, field) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (!validTypes.includes(file.type)) {
        setError(`Please upload only JPG, JPEG, or PNG files for ${field.replace('upload', '').replace('Id', ' ID').toLowerCase()}`);
        return;
      }
      ...
      if (file.size > maxSize) {
        setError(`File size must be less than 5MB for ${field.replace('upload', '').replace('Id', ' ID').toLowerCase()}`);
        return;
      }
      ...
      reader.onerror = () => {
        setError(`Failed to read image file for ${field.replace('upload', '').replace('Id', ' ID').toLowerCase()}`);
  ...
  const nextStep = async () => {
    ...
    const step1Fields = ["name", "email", "phone", "password", "confirmPassword", "dateOfBirth", "gender"];
    ...
    if (!hasStep1Errors && isStep1Valid()) {
      ...
      setError("Please fix the errors above before proceeding to the next step.");
    }
  };
      ...
      <Card className="overflow-hidden shadow-2xl border-0 rounded-3xl bg-gradient-to-br from-blue-50 via-white to-indigo-50 relative">
        ...
        <div className="relative p-8 md:p-10">
            ...
            <div className={currentStep === 2 ? "hidden" : "space-y-6"}>
              ...
              <div className="space-y-4">
                ...
                />
                    <ErrorMessage error={formik.touched.name && formik.errors.name ? (
                      <span id="name-error" className="flex items-center gap-1 text-red-500 text-sm mt-1" role="alert">
              ...
              <div className="space-y-4">
                ...
                <Input
                  ...
                  onBlur={formik.handleBlur}
                />
                    <ErrorMessage error={formik.touched.phone && formik.errors.phone ? (
                      <span className="flex items-center gap-1 text-red-500 text-sm mt-1">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M21 12A9 9 0 11 3 12a9 9 0 0118 0z" />
                        </svg>
                  ...
                  />
                    ...
                    <ErrorMessage error={formik.touched.dateOfBirth && formik.errors.dateOfBirth ? (
                      <span className="flex items-center gap-1 text-red-500 text-sm mt-1">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M21 12A9 9 0 11 3 12a9 9 0 0118 0z" />
                        </svg>
                  ...
                  Gender <span className="text-red-500">*</span>
                    ...
                    <div className="space-y-2 flex flex-row gap-4 w-full">
                      <div className="flex items-center flex-row space-x-3 h-10 px-3 rounded-lg border-2 border-gray-300 transition-all w-full">
                        ...
                        />
                        <Label htmlFor="gender-male" className="text-sm font-medium text-gray-700 cursor-pointer">
                          Male
                        </Label>
                      </div>
                      <div className="flex items-center flex-row space-x-3 h-10 px-3 rounded-lg border-2 border-gray-300 transition-all w-full">
                        <input
                          ...
                          className="h-4 w-4 text-primary focus:ring-primary border-gray-300"
                        />
                        <Label htmlFor="gender-female" className="text-sm font-medium text-gray-700 cursor-pointer">
                          Female
                        </Label>
                      </div>
                    </div>
              ...
              <div className="space-y-4">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (formik.errors.confirmPassword && e.target.value === formik.values.password) {
                ...
                {/* Password match indicator */}
                    ...
                    <ErrorMessage error={formik.touched.confirmPassword && formik.errors.confirmPassword ? (
                      <span id="confirmPassword-error" className="flex items-center gap-1 text-red-500 text-sm mt-1" role="alert">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M21 12A9 9 0 11 3 12a9 9 0 0118 0z" />
                        </svg>
            ...
            <div className={currentStep === 1 ? "hidden" : "space-y-6"}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-2">
                  ...
                  >
                    {formik.values.uploadFrontId && formik.values.uploadFrontId.length > 0 && imagePreviews.uploadFrontId ? (
                    ...
                    ) : formik.values.uploadFrontId && formik.values.uploadFrontId.length > 0 ? (
                  ...
                  <ErrorMessage error={formik.touched.uploadFrontId && formik.errors.uploadFrontId ? (
                    <span className="flex items-center gap-1 text-red-500 text-sm mt-1">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M21 12A9 9 0 11 3 12a9 9 0 0118 0z" />
                      </svg>
                ...
                <div className="space-y-2">
                  ...
                  <label
                    ...
                    className="relative flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer bg-white/80 backdrop-blur-sm hover:bg-white/90 transition-all hover:border-[var(--primary)] overflow-hidden group"
                  >
                    {formik.values.uploadBackId && formik.values.uploadBackId.length > 0 && imagePreviews.uploadBackId ? (
                      <div className="relative w-full h-full">
                        <img 
                    ...
                    ) : formik.values.uploadBackId && formik.values.uploadBackId.length > 0 ? (
                      <div className="flex items-center justify-center space-x-2 p-4">
                        <div className="flex items-center space-x-2">
                          <div className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center">
                            <span className="text-xs text-gray-600">📄</span>
                  ...
                  <ErrorMessage error={formik.touched.uploadBackId && formik.errors.uploadBackId ? (
                    <span className="flex items-center gap-1 text-red-500 text-sm mt-1">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M21 12A9 9 0 11 3 12a9 9 0 0118 0z" />
                      </svg>
                ...
                <div className="space-y-2">
                  ...
                  <label
                    ...
                    className="relative flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer bg-white/80 backdrop-blur-sm hover:bg-white/90 transition-all hover:border-[var(--primary)] overflow-hidden group"
                  >
                    {formik.values.uploadWorkId && formik.values.uploadWorkId.length > 0 && imagePreviews.uploadWorkId ? (
                      <div className="relative w-full h-full">
                        <img 
                    ...
                    ) : formik.values.uploadWorkId && formik.values.uploadWorkId.length > 0 ? (
                      <div className="flex items-center justify-center space-x-2 p-4">
                        <div className="flex items-center space-x-2">
                          <div className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center">
                            <span className="text-xs text-gray-600">📄</span>
                  ...
                  <ErrorMessage error={formik.touched.uploadWorkId && formik.errors.uploadWorkId ? (
                    <span className="flex items-center gap-1 text-red-500 text-sm mt-1">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M21 12A9 9 0 11 3 12a9 9 0 0118 0z" />
                      </svg>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onBlur={formik.handleBlur}
                />
                <ErrorMessage error={formik.touched.nationalId && formik.errors.nationalId ? (
                  <span className="flex items-center gap-1 text-red-500 text-sm mt-1">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M21 12A9 9 0 11 3 12a9 9 0 0118 0z" />
                    </svg>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-start space-x-4 p-4 bg-gradient-to-br from-blue-50 via-white to-indigo-50 rounded-lg border border-primary/80">
                  ...
                  <div className="flex-1">
                    <label htmlFor="acceptPolicy" className="-ms-2 text-sm text-gray-700 leading-relaxed">
                    ...
                    <p className="text-xs -ms-2 text-gray-500 mt-1">
                      By checking this box, you confirm that you understand and accept our terms and conditions.
                    </p>
                  </div>
                </div>
                <ErrorMessage error={formik.touched.acceptPolicy && formik.errors.acceptPolicy ? (
                  <span className="flex items-center gap-1 text-red-500 text-sm mt-1">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01M21 12A9 9 0 11 3 12a9 9 0 0118 0z" />
                    </svg>
          ...
          <div className="mt-5 text-center text-sm" style={{ color: 'var(--muted-foreground)' }}>
            ...
            <a href="/auth/login" className="font-semibold text-primary hover:text-primary-hover transition-colors duration-150 focus:outline-none rounded underline-offset-2 hover:underline">
              Sign In
            </a>
          </div>
        </div>
