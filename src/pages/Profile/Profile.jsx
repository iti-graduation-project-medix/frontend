...
export default function Profile() {
  ...
  const currentDeals = userData?.deals?.slice(indexOfFirstDeal, indexOfLastDeal) || [];
  const totalPages = Math.ceil((userData?.deals?.length || 0) / dealsPerPage);
  ...
  const { user, deals, pharmacies } = userData;
  ...
  const dealsByStatus = [
    { name: 'Active', value: deals.filter(deal => !deal.isClosed).length, color: '#10b981' },
    { name: 'Closed', value: deals.filter(deal => deal.isClosed).length, color: '#ef4444' }
  ];
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-6">
              <Avatar className="h-20 w-20 sm:h-20 sm:w-20">
                ...
                <AvatarFallback className="text-xl sm:text-2xl">
                  {userDetails.fullName?.charAt(0) || userDetails.email?.charAt(0) || 'U'}
                </AvatarFallback>
              </Avatar>
              <div className="flex-1 min-w-0 text-center sm:text-left">
                ...
                <div className="flex flex-wrap justify-center sm:justify-start items-center gap-2 mt-3">
                  <Badge variant={userDetails.isIdVerified ? "success" : "destructive"} className="text-xs">
                    {userDetails.isIdVerified ? "Verified" : "Unverified"}
                  </Badge>
                  <Badge variant={userDetails.isWorkIdVerified === "confirmed" ? "success" : "secondary"} className="text-xs">
                    {userDetails.isWorkIdVerified === "confirmed" ? "Work ID Verified" : "Work ID Pending"}
                  </Badge>
                  <Badge variant={userDetails.subscriptionStatus ? "success" : "outline"} className="text-xs">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader className="p-6 pb-4">
                  ...
                  <CardDescription>Distribution of active vs closed deals</CardDescription>
                </CardHeader>
                    ...
                    <PieChart>
                      ...
                      >
                        {dealsByTypeData.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={['#4a4957', '#4f46e5', '#ffb500'][index % 3]} />
                        ))}
                      </Pie>
                      <Tooltip />
                    </PieChart>
            ...
            <Card>
              ...
              <CardContent className="p-6 pt-0">
                        ...
                        <tr key={deal.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="p-4">
                            <Badge 
                              variant="outline"
                              className={
                                deal.dealType === "both" ? "bg-[#4a4957] text-white border-[#4a4957]" :
                                deal.dealType === "sell" ? "bg-[#4f46e5] text-white border-[#4f46e5]" :
                                deal.dealType === "exchange" ? "bg-[#ffb500] text-white border-[#ffb500]" :
                          ...
                          <td className="p-4 font-medium">EGP{Number(deal.price).toFixed(2)}</td>
                          <td className="p-4">
                            <Badge variant={deal.isClosed ? "destructive" : "success"}>
                ...
                <div className="md:hidden space-y-4">
                  {currentDeals.map((deal) => (
                    <div key={deal.id} className="border rounded-lg p-4 bg-white hover:bg-gray-50">
                      <div className="space-y-3">
                        {/* Medicine Info */}
                        <div>
                          <h3 className="font-semibold text-gray-900">{deal.medicineName}</h3>
                          <p className="text-sm text-gray-600">{deal.description}</p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          ...
                          <div>
                            ...
                            <p className="font-medium text-gray-900">EGP{Number(deal.price).toFixed(2)}</p>
                          </div>
                          <div>
                            <span className="text-gray-500 text-xs">Status</span>
                            <div className="mt-1">
                              <Badge 
                                variant={deal.isClosed ? "destructive" : "success"}
                              >
                                {deal.isClosed ? "Closed" : "Active"}
                              </Badge>
                            </div>
                ...
                {totalPages > 1 && (
                  <div className="mt-6 flex items-center justify-between">
                    <div className="text-sm text-gray-600">
                      Showing {indexOfFirstDeal + 1} to {Math.min(indexOfLastDeal, deals?.length || 0)} of {deals?.length || 0} deals
                    </div>
          ...
          <TabsContent value="pharmacies" className="space-y-6">
            ...
            <Card>
              <CardHeader className="p-6 pb-4">
                ...
                <CardDescription>Detailed information about each pharmacy</CardDescription>
              </CardHeader>
              <CardContent className="p-6 pt-0">
                    ...
                    <div key={pharmacy.id} className="border rounded-lg p-4">
                      <div className="flex justify-between mb-4">
                       <div className="flex flex-row  space-x-2">
                        ...
                        <Badge variant={pharmacy.isForSale ? "success" : "outline"}>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex items-center space-x-2">
                          ...
                          <div className="flex justify-between flex-1">
                            ...
                            <span className="font-medium">{pharmacy.licenseNum}</span>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                          ...
                          <div className="flex justify-between flex-1">
                            ...
                            <span className="font-medium">{pharmacy.pharmacyPhone}</span>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                          ...
                          <div className="flex justify-between flex-1">
                            ...
                            <span className="font-medium">{pharmacy.startHour} - {pharmacy.endHour}</span>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                          ...
                          <div className="flex justify-between flex-1">
                            <span className="text-gray-600 font-medium">Monthly Sales:</span>
                            <span className="font-medium">EGP{pharmacy.monthlySales}</span>
                          </div>
                        </div>
                        {pharmacy.isForSale && (
                          <div className="flex items-center space-x-2">
                            ...
                            <div className="flex justify-between flex-1">
                              ...
                              <span className="font-medium text-green-600">EGP{pharmacy.pharmacyPrice}</span>
                            </div>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="mt-4">
                        ...
                        <div className="ml-7 space-y-1">
                          ...
                          <p className="text-sm text-gray-600">
                            {pharmacy.city}, {pharmacy.governorate} {pharmacy.zipCode}
                          </p>
                        </div>
                      </div>
